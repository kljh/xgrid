...
function range_regression_tests() {
	...
	assert.equal(JSON.stringify(parse_range('R[1]')),
		'{"row":"1","abs_row":false,"abs_col":true}');
	assert.equal(JSON.stringify(parse_range('R[1]C[1]')),
		'{"row":"1","abs_row":false,"col":"1","abs_col":false}');
	assert.equal(JSON.stringify(parse_range('AA5')),
		'{"col":27,"abs_col":false,"row":5,"abs_row":false}');
	
	var aa5 = parse_range("AA5");
	assert.equal(stringify_range(aa5), 'AA5');
}
...
function parse_column_index_to_name(pos) {
	...
	do {
		var u = pos%26;
		var pos = (pos-u)/26;
		col = String.fromCharCode(A+u-1) + col;
	} while (pos!=0)
	return col;
}
...
if (typeof module!="undefined") {
	...
	module.exports.info_msg = info_msg;
